...
export function getIndex(el: Element) {
  ...
  return (
    1 + Array.from(el.parentElement.children).findIndex((child) => child === el)
  );
}
...
export function nodePath(...path: Element[]): HTMLElement[] {
  while (
    path[path.length - 1].parentElement != null ||
  ...
  /** 只需要是HTMLElement的 */
  const HTMLElementPath = <HTMLElement[]>(
    path.filter((el) => el instanceof HTMLElement)
  );
...
export namespace SelectionEvent {
  ...
  export function 高亮(
    options: { textDecoration?: string; style?: string } = {}
  ) {
...
namespace getIntermediateNodes {
  ...
  export function 获取两元素之间的元素(a: Node, b: Node) {
    ...
    /** 找出 a 与 b 在共存层的父元素 */
    const 短链路 =
      aParentList.length > bParentList.length ? bParentList : aParentList;
    let n1 = 短链路[0];
    let n2 = 短链路[0];
...
function getPath(element: Element) {
  ...
  while (element !== null && element.nodeType === Node.ELEMENT_NODE) {
    ...
    if (element.id) {
      ...
      let sibling = element, // Walk backwards until there is no previous sibling
